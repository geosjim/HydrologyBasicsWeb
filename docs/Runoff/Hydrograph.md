...
Peak Flow - The peak flow rate is defined as the maximum flow rate on the hydrograph and will occur when runoff is fully
developed.
...
Base Flow - The base flow is the part of the hydrograph that represents the normal flow in a perennial stream, or in
other words the flow in a river before the rainfall event.
